...
Rotation._readField = function (tag, obj, pbf) {
    ...
    else if (tag === 3) obj.w = pbf.readVarint(true);
};
Rotation.write = function (obj, pbf) {
    if (obj.x) pbf.writeVarintField(0, obj.x);
    if (obj.y) pbf.writeVarintField(1, obj.y);
    if (obj.z) pbf.writeVarintField(2, obj.z);
    if (obj.w) pbf.writeVarintField(3, obj.w);
};
...
TickData.write = function (obj, pbf) {
    ...
    if (obj.bodies) for (var i = 0; i < obj.bodies.length; i++) pbf.writeMessage(1, TickData.TickBodyData.write, obj.bodies[i]);
};
...
TickData.TickBodyData.read = function (pbf, end) {
    return pbf.readFields(TickData.TickBodyData._readField, {id: 0, position: null, rotation: null, updateTime: 0, health: 0, lastShotTime: 0}, end);
};
