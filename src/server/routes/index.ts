...
export function applyRouter(app: Express, state: State) {
  ...
  app.post('/register', (req, res) => {
    ...
    if (!req.body.username) { return res.status(422).send('Unprocessed entity'); }
    if (!req.body.password) { return res.status(422).send('Unprocessed entity'); }
    if (!req.body.sessionId) { return res.status(422).send('Unprocessed entity'); }
